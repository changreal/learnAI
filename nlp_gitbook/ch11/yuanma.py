...
class PoetryModel(object):
    ...
    def build_model(self):
        ...
        # 输入的dimension
        input_tensor = Input(shape=(self.config.max_len,))  # shape=(?,6)
        embedd = Embedding(len(self.num2word) + 2, 300, input_length=self.config.max_len)(input_tensor)  # shape=(?, 6, 300)
        lstm = Bidirectional(GRU(128, return_sequences=True))(embedd)  # shape=(?,?,256)
    ...
    def data_generator(self):
        ...
        while 1:
            ...
            y_vec = np.zeros(
                shape=(1, len(self.words)), # 有5790个词
                dtype=np.bool
            )
    ...
    def train(self):
        '''训练模型'''
        number_of_epoch = len(self.files_content) // self.config.batch_size # 96617次迭代
